 ...
 */
export async function generateUserInsights(input) {
...
function calculateSummary(moodHistory) {
    ...
    const variance = moodScores.reduce((sum, score) => sum + Math.pow(score - meanMood, 2), 0) / moodScores.length;
...
function detectPatterns(moodHistory) {
    const patterns = [];
    // Temporal patterns
    ...
    if (moodCyclePattern)
        patterns.push(moodCyclePattern);
    // Energy patterns
...
function detectTemporalPatterns(moodHistory) {
...
function detectMoodCycles(moodHistory) {
    ...
    if (swingFrequency > 30) {
        return {
            ...
            confidence: 80,
            recommendation: "Consider establishing daily routines and stability practices like regular sleep schedules and meditation",
        };
    }
    ...
    if (lowPeriods > 0) {
        return {
            ...
            confidence: 85,
            recommendation: "Consider reaching out to a mental health professional for additional support during these periods",
        };
    }
    return null;
}
function detectEnergyPatterns(moodHistory) {
    const avgEnergy = moodHistory.reduce((sum, entry) => sum + entry.energyLevel, 0) / moodHistory.length;
    if (avgEnergy < 4) {
        return {
            type: "energy",
            ...
            frequency: 100,
            confidence: 90,
            recommendation: "Focus on sleep quality, nutrition, hydration, and consider checking in with a doctor about persistent fatigue",
        };
    }
    ...
    if (correlation > 70) {
        return {
            ...
            confidence: 85,
            recommendation: "Focus on energy management: regular exercise, consistent sleep, and balanced nutrition will help improve mood",
        };
    }
    return null;
}
function detectTriggerPatterns(moodHistory) {
    const triggerCounts = {};
...
function analyzeTriggers(moodHistory) {
    const triggerStats = {};
    moodHistory.forEach((entry) => {
        if (entry.triggers) {
            entry.triggers.forEach((trigger) => {
...
function generateRecommendations(summary, patterns, triggers) {
    const recommendations = [];
    // Trend-based recommendations
    if (summary.improvementTrend === "declining") {
        recommendations.push("Your mood has been declining recently. Consider reaching out to a mental health professional for support.");
        recommendations.push("Increase self-care activities and ensure you're getting adequate sleep, nutrition, and social connection.");
    }
    else if (summary.improvementTrend === "improving") {
        recommendations.push("Your mood is improving! Keep up the practices that are working for you.");
    }
    // Mood stability recommendations
    if (summary.moodStability < 50) {
        recommendations.push("Your mood varies significantly. Establishing daily routines can help create more stability.");
    }
    // Energy recommendations
    if (summary.averageEnergy < 5) {
        recommendations.push("Your energy levels are low. Prioritize sleep, regular movement, and balanced nutrition.");
    }
    ...
    if (negativeTriggers.length > 0 && recommendations.length < 5) {
        recommendations.push(`Develop coping strategies for recurring stressors: ${negativeTriggers.map((t) => t.trigger).join(", ")}`);
    }
    return recommendations.slice(0, 5);
}
function generateInsights(summary, patterns, triggers, moodHistory) {
    const insights = [];
    // Summary insights
    insights.push(`Over the analyzed period, your average mood was ${summary.averageMood}/10 with an average energy level of ${summary.averageEnergy}/10.`);
    insights.push(`Your most common emotional state has been "${summary.mostCommonMood}".`);
    if (summary.moodStability >= 70) {
        insights.push("Your mood has been relatively stable, which is a positive indicator of emotional regulation.");
    }
    else if (summary.moodStability < 50) {
        insights.push("Your mood fluctuates significantly, which may indicate external stressors or need for more stability practices.");
    }
    // Pattern insights
    if (patterns.length > 0) {
        insights.push(`Key pattern identified: ${patterns[0].description.toLowerCase()}`);
    }
    // Trigger insights
    if (triggers.length > 0) {
        const topTrigger = triggers[0];
        insights.push(`Your most frequent trigger is "${topTrigger.trigger}" (${topTrigger.frequency.toFixed(0)}% of entries).`);
    }
    // Best and worst days
    const sortedByMood = [...moodHistory].sort((a, b) => b.moodScore - a.moodScore);
    if (sortedByMood.length >= 2) {
        const bestDay = sortedByMood[0];
        insights.push(`Your best day was ${new Date(bestDay.date).toLocaleDateString()} with a mood score of ${bestDay.moodScore}/10.`);
    }
...
//# sourceMappingURL=user-insights.js.map